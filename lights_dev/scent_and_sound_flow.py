...
from game.constants import FlowType, MAX_FLOWS
...
def cave_closed_door(feature_type: int) -> bool:
    ...
    """
    return feature_type == FeatureType.CLOSED_DOOR or feature_type == FeatureType.SECRET_DOOR
...
def _propagate_noise_kernel(
    ...
    # Preallocated queue arrays.  max_q = total cells; worst-case queue length.
    max_q = height * width
    qy = np.empty(max_q, dtype=np.int32)
    qx = np.empty(max_q, dtype=np.int32)
    qc = np.empty(max_q, dtype=np.int32)
    head = 0
    tail = 0
    ...
    # BFS/Dijkstra-like propagation (first-visit wins with relaxation)
    while head < tail:
        y = qy[head]
        x = qx[head]
        current_cost = qc[head]
        head += 1
        ...
        # If grid already holds a better value, skip relax from this node
        if cost_grid[y, x] < current_cost:
            continue
        ...
        for idx in range(8):
            ...
            if is_closed:
                if flow_type_val == int(FlowType.NO_DOORS):
                    # Blocked for this flow type
                    continue
                elif flow_type_val == int(FlowType.PASS_DOORS):
                    cost_increase += door_pass_penalty
                elif (flow_type_val == int(FlowType.REAL_NOISE)
                      or flow_type_val == int(FlowType.MONSTER_NOISE)):
            ...
            if new_cost < cost_grid[ny, nx]:
                ...
                # enqueue if space remains (tail capped at max_q)
                if tail < max_q:
                    qy[tail] = ny
                    qx[tail] = nx
                    qc[tail] = new_cost
                    tail += 1
                # If tail >= max_q, we drop enqueue; this is extremely unlikely for
                # reasonable map sizes and acts as a safety net to avoid overflow.
...
def terrain_transparency_map(terrain_map: NDArray[np.int32]) -> NDArray[np.bool_]:
    ...
    """
    return terrain_map != FeatureType.WALL
...
# -----------------------------------------------------------------------------
def _sil_skill_check_with_rng(skill: int, difficulty: int, opposition: int, rng: GameRNG) -> bool:
    """
...
def monster_perception(
    ...
    # Parallel path
    df_chunks = [active_monsters[i : i + chunk_size] for i in range(0, active_monsters.height, chunk_size)]
...
def choose_step_by_flow(
    ...
    for dy, dx in NEIGHBORS_8:
        ...
        if not (0 <= ny < h and 0 <= nx < w):
            continue
        # Skip walls
        if game_tiles[ny, nx] == FeatureType.WALL:
            continue

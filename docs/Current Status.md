...
| Overland generation | `worldgen/overland/`, `tools/generate_overland.py`, `tools/inspect_overland.py` | Evolving integrated subsystem | `python tools/generate_overland.py --help` | Overland-first terrain, hydrology, route, transition, affordance, and settlement-merge contract; planning source is `docs/Overland Roadmap.md` | Hydrology continuity and stable `flow_group` semantics are the next implementation slice. |

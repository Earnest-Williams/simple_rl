...
| First playable expedition loop | `game/expedition/`, `game/systems/survey.py`, `game/systems/repair.py`, `tools/play_game.py --first-playable` | Integrated | `python tools/play_game.py --first-playable` | Launches the ruined-harbor vertical slice with survey, route reveal, blockage repair, cave transition, and return-to-port completion | Expand objective variety and cave interiors after the current slice |

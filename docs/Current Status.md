...
| Overland generation | `worldgen/overland/`, `tools/generate_overland.py`, `tools/inspect_overland.py` | Evolving integrated subsystem | `python tools/generate_overland.py --help` | Overland-first terrain, connected hydrology, starting-region contract (with RouteSegmentState, evidence_tags, repair_cost), runtime OverlandMapMetadata sidecar in convert.py/GameMap, deterministic repair simulation in routes.py, settlement roads using CLEAR/REPAIRED states, actor_traversal.py now consults metadata for state-based costs (REPAIRED cheaper, BLOCKED blocked); Option B complete. Phase 5+ verified. Planning source is `docs/Overland Roadmap.md`. | None (next: richer evidence in runtime survey systems or full integration into game/world/game_map.py). |

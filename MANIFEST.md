...
| `game/systems/ai_system.py` | Central AI dispatch selecting adapters and executing in parallel thread pools | `collections.abc.Iterable`<br>`multiprocessing.dummy.Pool`<br>`structlog`<br>`game.ai` (get_adapter, goap) | `batch_size` - 4 (default thread pool size) |
...
| `magic/bf_backend.py` | Abstract and concrete Brainfuck backends with Numba JIT and pure Python | `annotations`, `ABC`, `abstractmethod`, `dataclass`<br>`brainfuck_numba` | Tape size: 30,000<br>Max steps: 10,000,000 |
| `magic/brainfuck_numba.py` | High-performance BF interpreter with Numba JIT, sandboxing, safety limits | `resource`, `time`, `multiprocessing`, `numpy`<br>`numba.njit`, `bf_backend.BFResult` | CMD chars (ASCII 43-93)<br>SANDBOX_STEP_THRESHOLD - 1,000,000<br>SANDBOX_CPU - 1s<br>SANDBOX_WALL_TIME - 1.0s<br>SANDBOX_MEMORY - 256MB<br>Max output - 1,000,000 bytes |
...
| `pathfinding/perception_systems.py` | Noise, smell, perception systems for RL agents with NumPy/Numba/Polars optimization | `logging`, `os`, `collections.deque`, `enum.IntEnum`<br>`numpy`, `polars`, `joblib` (Parallel, delayed)<br>`numba.njit`, `common.constants.FeatureType`<br>`common.types`, `game.world.los`, `utils.game_rng.GameRNG` | MAP_HGT/WID - 64<br>BASE_FLOW_CENTER - 100<br>NOISE_STRENGTH - 80<br>NOISE_MAX_DIST - 200<br>SMELL_STRENGTH - 80<br>SCENT_RESET_AGE - 250<br>N_JOBS - cpu_count//2<br>MAX_FLOWS - 4<br>NEIGHBORS_8 - 8 tuples<br>SCENT_ADJUST_TABLE - 5×5 array [0,1,2,250] |
...
## legacy/, lights_dev/, simulation/ Directories

| File | Purpose | Imports | Constants/Magic Numbers |
|------|---------|---------|------------------------|
| `legacy/dungeon_generator.py` | Adapter shim for fallback dungeon generation, loads external or uses internal BSP | `contextlib`, `importlib`, `os`, `warnings`, `lru_cache`, `ModuleType`, `Any` | _DEFAULT_CANDIDATES - module names<br>max_attempts - max(1000, rooms*50)<br>margin - 2<br>Bresenham line drawing |
| `legacy/simple_rl.py` | Game loop with player movement/combat, dungeon gen, entities, scripting macros, GameRNG | `json`, `os`, `sys`, `warnings`, `Any`<br>`PySide6` (QtCore, QtGui, QtWidgets)<br>`dungeon_generator`, `MacroManager`, `GameRNG`, `MetricsCollector` | ROOM_MIN_SIZE - 3<br>ROOM_MAX_SIZE - 5<br>NUM_ROOMS - 12<br>DUNGEON_WIDTH/HEIGHT - 40<br>PLAYER_HP - 50<br>SKELETON_HP - 5<br>PLAYER_DAMAGE - (1,5)<br>SKELETON_DAMAGE - (1,3)<br>ROOM_GAP - 3<br>GAME_SEED - 42<br>Key mappings |
| `lights_dev/constants.py` | Global constants for FOV/light/memory: tile IDs, characters, RGB colors, ANSI codes, decay | None | MAX_LOS_DISTANCE - 500<br>WALL_ID - 0, FLOOR_ID - 1, PILLAR_ID - 2<br>MEMORY_DURATION - 60.0<br>MEMORY_SIGMOID_MIDPOINT - 30.0<br>MEMORY_SIGMOID_STEEPNESS - 0.1<br>MEMORY_LEVEL_COUNT - 5<br>MAX_LIGHT_LEVEL_FOR_VIS_CHECK - 6<br>LIGHT_LEVEL_FALLOFF_RATE - 5<br>Colors: TORCH-(255,160,60), ORB-(160,200,255), AMBIENT-(30,30,45) |
...
| `skills/models.py` | 29-skill IntEnum, training configs, dataclasses with strict typing | `dataclasses`, `enum.IntEnum`, `typing.Final`, `numpy` | SKILL_COUNT - 29<br>MAX_SKILL_LEVEL - 27<br>MIN_APTITUDE - -5<br>MAX_APTITUDE - 11<br>XP_FORMULA_CONSTANT - 25<br>Cross-training: 0.40 (high), 0.25 (medium), 0.10 (low) |
...
- **Grid sizes**: 15, 64, 100, 128, 200 (various subsystems)
- **Skill max level**: 27 (DCSS-inspired)
- **Tape size**: 30,000 (Brainfuck interpreter)
- **Max steps**: 10,000,000 (Brainfuck safety limit)
- **Sample rates**: 22050 Hz (music), 44100 Hz (SFX)
- **Memory duration**: 60-90 seconds (memory fade)

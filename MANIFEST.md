...
| `game/world/fov.py` | Production Numba FOV plus callback-driven visibility APIs using iterative/symmetrical shadowcasting with height/ceiling checks | `time`, `deque`, `collections.abc`<br>`numba`, `numpy`, `structlog`<br>`los` - Line of sight | `BASE_THRESHOLD` - 1 (height diff baseline)<br>`CLOSE_RANGE_SQ_THRESHOLD` - 16<br>`CLOSE_RANGE_DIVISOR` - 8<br>`FAR_RANGE_DIVISOR` - 16<br>`_THRESHOLD_AT_CUTOFF` - 2 (16//8)<br>`MAX_SECTORS` - 10000 (safety limit) |
| `game/world/game_map.py` | Core map with tiles, FOV, visibility/exploration tracking, memory fading, height/ceiling | `math`, `dataclass`, `pathlib.Path`<br>`typing` (Final, NamedTuple)<br>`numpy`, `structlog`, `yaml`<br>`tile_id_for`, `compute_visibility_into`, `update_memory_fade` | `MAX_MEMORY_STRENGTH` - 5.0<br>TILE_ID_FLOOR/WALL computed<br>Default floor: "blank_tile_a", wall: "wall_stone_bricks"<br>Colors: FG (200,200,200)/(180,180,180), BG (10,10,30)/(30,30,50)<br>Memory modifier: 1.0 floor, 2.0 wall |
...
| File | Purpose | Imports | Constants/Magic Numbers |
|------|---------|---------|------------------------|
| `lights_dev/__main__.py` | Python module entrypoint that delegates to CLI runner | `lights_dev.cli.run_simulation` | None |
| `lights_dev/_numba_fov.py` | Shared Numba-jitted octant traversal, coordinate transforms, slope jitclass, and memory fade kernel used by FOV and memory systems | `math`, `numba`, `numpy`, `typing.Final`<br>`lights_dev.constants`, `lights_dev.dungeon_data.Dungeon` | `slope_spec` - jitclass fields (y, x as int64)<br>Slope jitclass with comparison methods<br>Octant direction vectors and coordinate transforms |
| `lights_dev/cli.py` | Debug/profiling CLI runner that wraps GameRunner with logging, optional readchar interaction, and frame timing | `logging`, `os`, `time`<br>`lights_dev.constants`, `lights_dev.game_state.find_path`, `lights_dev.runner.GameRunner`<br>`readchar` (optional) | `READCHAR_AVAILABLE` - bool flag<br>target_duration - 60s (profiler) or 300s (interactive)<br>profiler_move_delay - 0.01s<br>Frame timing thresholds |
| `lights_dev/constants.py` | Global constants for FOV/light/memory: tile IDs, characters, RGB colors, ANSI codes, decay | `common.tuning.MEMORY_LEVEL_COUNT` - Centralized memory level count | MAX_LOS_DISTANCE - 500<br>WALL_ID - 0, FLOOR_ID - 1, PILLAR_ID - 2<br>MEMORY_DURATION - 60.0<br>MEMORY_SIGMOID_MIDPOINT - 30.0<br>MEMORY_SIGMOID_STEEPNESS - 0.1<br>`MEMORY_LEVEL_COUNT` - 5 (imported from common.tuning)<br>MAX_LIGHT_LEVEL_FOR_VIS_CHECK - 6<br>LIGHT_LEVEL_FALLOFF_RATE - 5<br>Colors: TORCH-(255,160,60), ORB-(160,200,255), AMBIENT-(30,30,45)<br>DEBUG_RENDER_MODE - "normal"<br>DEFAULT_ENTITY_CATEGORY - "medium"<br>TILE_ID_TO_CATEGORY mapping |
| `lights_dev/demo_dungeon_generator.py` | Simple test map generation (U-shape, rooms) for lights_dev testbed | `numpy`, `utils.game_rng.GameRNG`<br>`lights_dev.constants`, `lights_dev.dungeon_data.Dungeon` | Various map generation dimensions and patterns |
...
| `lights_dev/entities.py` | Entity/Player/LightSource classes with RNG-injected flicker and path following | `lights_dev.constants`<br>`utils.game_rng.GameRNG` | Player default light_radius - 10<br>Player default light_level - 3<br>LightSource default radius - 16<br>LightSource default level - 5<br>Flicker probability - 0.2<br>Radius variance - ±3 |
| `lights_dev/fov.py` | FOVSystem wrapper providing compute_fov() and precompile() APIs; delegates to shared Numba octant traversal from _numba_fov | `math`, `typing.Final`, `numba`, `numpy`<br>`numba` type imports (boolean, float32, uint8, uint32)<br>`lights_dev._numba_fov` (Slope, _compute_octant_for_boolean_array)<br>`lights_dev.dungeon_data.Dungeon` | SIDE_N/E/S/W/NE/SE/SW/NW bits (1<<0 through 1<<7)<br>INT - numba.int64<br>DUMMY_CELL_MASK - empty uint32 array<br>opacity_threshold - 0.999999 |
| `lights_dev/game_state.py` | GameState orchestrator managing dungeon, entities, FOV, lighting, and memory updates | `logging`, `collections.deque`, `numba`, `numpy`, `numpy.typing.NDArray`<br>`lights_dev` (constants, demo_dungeon_generator, dungeon_data.Dungeon, entities, fov.FOVSystem, lighting.LightingSystem, memory.update_memory_fade)<br>`utils.game_rng.GameRNG` | min_brightness_for_vis threshold calculation<br>Player spawn search attempts - dungeon.height or dungeon.width * 2<br>Light source positions and configs |
| `lights_dev/lighting.py` | LightingSystem wrapper providing compute_illumination(), compute_final_rgb_map(), apply_lighting(), and precompile() APIs for directional lighting with per-side accumulation and height/incidence effects | `dataclass`, `typing.Iterable`, `math`, `numba`, `numpy`, `numpy.typing.NDArray`<br>`lights_dev.constants`<br>`lights_dev._numba_fov` (octant helpers) | SIDE_N through SIDE_NW - 0-7 indices<br>NUM_SIDES - 8<br>DEFAULT_CHANNEL_MASK - 0xFFFFFFFF<br>CORE_NO_ATTEN_RADIUS - 2.0<br>Coordinate system: screen coords (Y down, 0°=East, 90°=South) |
| `lights_dev/main_game.py` | Compatibility entrypoint that forwards to cli.run_simulation() | `lights_dev.cli.run_simulation` | None |
| `lights_dev/memory.py` | Memory fade system with update_memory_fade(), get_character_indices(), get_memory_character(), and precompile() APIs; delegates to shared _numba_fov kernel | `math`, `dataclasses` (dataclass, field), `typing.Final`, `numba`, `numpy`, `numpy.typing.NDArray`<br>`lights_dev.constants`, `lights_dev._numba_fov._update_memory_fade_internal`<br>`common.tuning.MEMORY_LEVEL_COUNT` - Centralized memory level count | BASE_MEMORY_DURATION - 90.0<br>BASE_SIGMOID_MIDPOINT - 45.0<br>BASE_SIGMOID_STEEPNESS - ~0.056<br>`MEMORY_LEVEL_COUNT` - 5 (imported from common.tuning)<br>MIN_INTELLIGENCE - 1<br>MAX_INTELLIGENCE - 30<br>BASE_INTELLIGENCE - 10<br>DEFAULT_UPDATE_INTERVAL - 0.1<br>MIN_INTENSITY_THRESHOLD - 0.001<br>_EXP_CLAMP - 70.0 |
| `lights_dev/renderer.py` | Renderer class providing render() API to convert GameState into ANSI True Color text frames with multiple debug modes | `numpy`<br>`lights_dev` (constants, entities.LightSource, game_state.GameState, lighting.LightingSystem, memory.get_memory_character) | Render modes: "normal", "level", "intensity", "level_color"<br>brightness_threshold - 0.001<br>memory_intensity_threshold - 0.01 (intensity mode), 0.0 (normal), 0.3 (memory light) |
| `lights_dev/runner.py` | GameRunner embedding API managing initialization, precompile, step(), and render() for front-end integration without logging config | `logging`, `time`, `typing.Callable`<br>`lights_dev` (constants, fov.FOVSystem, game_state.GameState, lighting.LightingSystem, memory.precompile, renderer.Renderer)<br>`utils.game_rng.GameRNG` | Default frame time clamp - 0.1s<br>Default run_loop FPS - 60<br>Seed fallback - int(time.time() * 1000) |
| `lights_dev/scent_and_sound_flow.py` | Noise flow propagation, scent stamping, and perception checks for AI sensing | `logging`, `os`, `typing.Final`<br>`numpy`, `polars`, `joblib`<br>`numba`, `numpy.typing.NDArray`<br>`FeatureType`, `FlowType`, `MAX_FLOWS`<br>`game.world.los.line_of_sight`, `utils.game_rng.GameRNG` | BASE_FLOW_CENTER - 100<br>NOISE_STRENGTH - 80<br>NOISE_MAX_DIST - 200<br>SMELL_STRENGTH - 80<br>SCENT_RESET_AGE - 250<br>NEIGHBORS_8 - 8-direction offsets<br>SCENT_ADJUST_TABLE - 5×5 scent stamp table |
| `simulation/__init__.py` | Exports ZoneManager for zone-based simulation | `simulation.zone_manager.ZoneManager` | None |
...
| `fonts/tree.txt` | Archival/diagnostic snapshot of the font directory structure; prefer `rg --files fonts` for current inventory | Directory listing |
...
- **Other**: pyproject.toml, environment.yml, .gitignore

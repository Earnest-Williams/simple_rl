...
def print_dungeon_layout(
    dungeon: Dungeon, light_x: int, light_y: int, target_x: int, target_y: int
) -> None:
    ...
    for tile_id in unique_tiles:
        ...
        tile_name = (
            ...
            if tile_id == constants.WALL_ID
                ...
                else "FLOOR" if tile_id == constants.FLOOR_ID else f"UNKNOWN({tile_id})"
            )
        )
...
def check_transparency_array(
    ...
    if np.any(bad_cells):
        ...
        for y, x in coords:
            print(
                f"   Cell ({x},{y}): opaque={opaque[y,x]}, transparency={transparency[y,x]:.4f}"
            )
...
def check_line_of_sight_leaks(
    ...
    if target_alpha > 0.01:
        ...
        if blocker_found:
            ...
            print(
                "⚠ Target has clear line of sight but is not lit (might be out of range)"
            )

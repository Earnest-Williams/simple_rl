...
from worldgen.overland.schema import RouteSegmentState, TraversalClass, EvidenceTag
...
def clear_blockage_at(gs: GameState, x: int, y: int) -> bool:
    ...
    if metadata is None:
        gs.add_message("No overland metadata available to perform repairs.", (255, 100, 100))
        return False
    ...
    if route_id:
        for seg in getattr(metadata, "route_segments", []) or []:
            if seg.get("route_id") == route_id:
                ...
                log.info("Route segment repaired", route_id=route_id, turn=gs.turn_count)
    ...
    if hasattr(metadata, "movement_cost_grid") and metadata.movement_cost_grid is not None:
        metadata.movement_cost_grid[y, x] = 1.0
    if hasattr(metadata, "traversal_class_grid") and metadata.traversal_class_grid is not None:
        metadata.traversal_class_grid[y, x] = int(TraversalClass.NORMAL)

    # Print success message to log
    gs.add_message(f"You successfully clear the blockage at ({x}, {y})!", (100, 255, 100))

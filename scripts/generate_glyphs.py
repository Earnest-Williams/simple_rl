...
ORIGINAL_SAFE_DUMP: Final[Callable[..., str | None]] = yaml.safe_dump


def safe_dump_with_dumper(
    data: object, stream: IO[str] | None = None, **kwargs: object
) -> str | None:
    if "Dumper" in kwargs:
        dumper_value = kwargs.pop("Dumper")
        if isinstance(dumper_value, type) and issubclass(
            dumper_value, yaml.SafeDumper
        ):
            return yaml.dump_all([data], stream, Dumper=dumper_value, **kwargs)
        raise TypeError("Dumper must be a SafeDumper subclass.")
    return ORIGINAL_SAFE_DUMP(data, stream, **kwargs)


setattr(yaml, "safe_dump", safe_dump_with_dumper)
...
class IndentDumper(yaml.SafeDumper):
    """SafeDumper that forces block indentation for readability."""

    def increase_indent(self, flow: bool = False, indentless: bool = False) -> None:
        # Always force indentation for nested structures (preserve block style)
        super().increase_indent(flow, False)


def write_yaml(path: Path, entries: Sequence[GlyphEntry]) -> None:
    """
    Write glyph metadata to `path` with stable, human-readable YAML.

    Behavior:
    * Uses yaml.safe_dump (no unsafe Python tags).
    * Uses sort_keys=False to keep insertion order; we build dict keys
      in the chosen canonical order.
    * Emits explicit empty alt_names as [] and notes as ''.
    """
    glyphs: list[dict[str, object]] = []
    for entry in entries:
        # canonical key insertion order: name, tile_id, png, svg, alt_names, notes, confirmed
        glyph: dict[str, object] = {
            "name": entry.name,
            "tile_id": entry.tile_id,
            "png": entry.png if entry.png is not None else None,
            "svg": entry.svg if entry.svg is not None else None,
            # explicit empty list/string so YAML always shows the key
            "alt_names": entry.alt_names if entry.alt_names else [],
            "notes": entry.notes if entry.notes else "",
            "confirmed": bool(entry.confirmed),
        }
        glyphs.append(glyph)

    # Write using safe_dump with deterministic formatting options
